...
/**
 * function to measure ADM. ADM has only 1 dc per each child(ex. child A has 1 DC, child B has 1 DC...)
 * Just simple MEAS DC(Pinlist) then close all DC relay of child.
 * if use ATL, then need to use "MULTIPLE DUT PARRALEL" or "MULTIPLE DUT SINGLE"
 * if use MCI, then need to execute the exclusion for not under measurement dut. 
 */
